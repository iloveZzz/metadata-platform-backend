...
CREATE TABLE IF NOT EXISTS tool_registry (
    ...
    enabled BOOLEAN NOT NULL DEFAULT TRUE,
    params_schema_ref VARCHAR(256)
);

MERGE INTO tool_registry (tool_name, version, enabled, params_schema_ref) KEY (tool_name) VALUES
('search_assets', '1.0.0', TRUE, 'mcp-tools-contract-draft.md#3.1'),
('asset_detail', '1.0.0', TRUE, 'mcp-tools-contract-draft.md#3.2'),
('lineage', '1.0.0', TRUE, 'mcp-tools-contract-draft.md#3.3'),
('impact_analysis', '1.0.0', TRUE, 'mcp-tools-contract-draft.md#3.4'),
('classification_query', '1.0.0', TRUE, 'mcp-tools-contract-draft.md#3.5');

CREATE TABLE IF NOT EXISTS audit_log (
    id VARCHAR(64) PRIMARY KEY,
    mcp_request_id VARCHAR(64),
    session_id VARCHAR(64),
    agent_id VARCHAR(64),
    tool VARCHAR(64),
    params_summary VARCHAR(1024),
    result_summary VARCHAR(1024),
    result_code VARCHAR(32),
    timestamp TIMESTAMP,
    duration_ms INT,
    internal_permission_flag VARCHAR(32)
);
