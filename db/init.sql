-- =====================================================================
-- 数据中台元数据发现与智能治理平台 (Metadata Platform) 全量数据库初始化脚本
-- 涵盖模块：
--   1. 基础服务 (Leaf 分布式 ID 发号配置)
--   2. 元数据资产目录与血缘 (MP-SLICE-01 ~ 08)
--   3. 数据质量中心 (DQ Insight, 01 ~ 04)
--   4. 智能治理引擎与指标对齐 (Smart Governance, SG-01 ~ 05)
--   5. 数据安全与动态脱敏中心 (Data Security, SEC-01 ~ 06)
--   6. 智能找数与资产打标 (Smart Discovery, SD-01 ~ 05)
--   7. 语义层与术语库 (Semantic Layer, SL-01 ~ 06)
--   8. AI Agent 上下文与 MCP 工具网关 (AI Context Layer, ACL-01 ~ 05)
-- =====================================================================
...
-- =====================================================================
-- 2. 元数据资产核心表 (Metadata Platform Core)
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS `data_source` (
    ...
    `type` VARCHAR(32) NOT NULL COMMENT '连接器类型（MySQL/Oracle/.../OSS/S3）',
    `host` VARCHAR(255) NOT NULL COMMENT '主机地址',
    `port` INT(11) NOT NULL COMMENT '端口',
    `dialect` VARCHAR(32) NOT NULL COMMENT '方言（native/mysql-compatible/gaussdb/auto）',
    `username` VARCHAR(128) DEFAULT NULL COMMENT '用户名',
    `cred_ref` VARCHAR(512) DEFAULT NULL COMMENT '凭据加密引用',
    `auto_classify` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '是否自动识别分类',
    `status` VARCHAR(32) NOT NULL DEFAULT 'draft' COMMENT '状态：draft/connected/failed/disabled',
...
CREATE TABLE IF NOT EXISTS `collector_task` (
    ...
    `schedule` VARCHAR(128) NOT NULL COMMENT '调度（cron 或周期描述）',
    `mode` VARCHAR(32) NOT NULL COMMENT '采集模式：incremental/full',
    `strategy` VARCHAR(32) NOT NULL DEFAULT 'ignore' COMMENT '覆盖策略：ignore/overwrite/abort-on-failure',
    `auto_classify` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '是否自动识别分类',
    ...
    `enabled` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '生效状态：1 生效，0 停用',
    `datasource_type` VARCHAR(64) DEFAULT NULL COMMENT '数据源类型',
    `source_system` VARCHAR(64) DEFAULT NULL COMMENT '来源系统',
    `scope_type` VARCHAR(32) DEFAULT 'all' COMMENT '采集范围：all/custom',
...
CREATE TABLE IF NOT EXISTS `asset` (
    ...
    `status` VARCHAR(32) NOT NULL DEFAULT 'pending' COMMENT '状态：待认领/已认领/已归档/已删除',
    `taint_status` VARCHAR(20) NOT NULL DEFAULT 'NORMAL' COMMENT '数据存疑状态：NORMAL/TAINTED',
...
CREATE TABLE IF NOT EXISTS `lineage_edge` (
    ...
    `confidence` VARCHAR(32) NOT NULL COMMENT '置信度：auto-high/auto-mid/manual-high/low',
    `remark` VARCHAR(512) DEFAULT NULL COMMENT '备注',
    `graph_version` VARCHAR(64) DEFAULT NULL COMMENT '图版本（并发防冲突 token）',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `export_task` (
    ...
    `asset_id` VARCHAR(36) DEFAULT NULL COMMENT '影响分析源资产（NULL=DataHub 全局导出）',
...
CREATE TABLE IF NOT EXISTS `classification` (
    ...
    `status` VARCHAR(32) NOT NULL DEFAULT 'pending' COMMENT '状态：待确认/已确认/已修正',
    PRIMARY KEY (`id`),
...
INSERT IGNORE INTO `data_domain` (`id`, `name`, `owner`) VALUES
('dom-001', '交易域', 'trader_admin'),
('dom-002', '客户域', 'crm_admin'),
('dom-003', '风控域', 'risk_admin'),
('dom-004', '清算域', 'settle_admin'),
('dom-005', '财务域', 'finance_admin');
...
INSERT IGNORE INTO `role` (`id`, `name`, `scope`) VALUES
('role-001', '平台管理员', 'GLOBAL'),
('role-002', '数据安全专员', 'SECURITY'),
('role-003', '数据质量专员', 'QUALITY'),
('role-004', '普通分析师', 'READ_ONLY');
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='审计日志表（不可变）';
...
CREATE TABLE IF NOT EXISTS `integration_config` (
    ...
    `gravitino_auth_ref` VARCHAR(512) DEFAULT NULL COMMENT 'Gravitino 认证（加密引用）',
    ...
    `datahub_auth_ref` VARCHAR(512) DEFAULT NULL COMMENT 'DataHub 认证（加密引用）',
...
CREATE TABLE IF NOT EXISTS `openlineage_event` (
    ...
    `parse_status` VARCHAR(16) NOT NULL DEFAULT 'received' COMMENT 'received/parsed/parse_failed',
...
-- =====================================================================
-- 4. 智能安全分类分级与指标对齐引擎 (Smart Governance)
-- =====================================================================
...
INSERT IGNORE INTO `sg_compliance_template` (`id`, `template_code`, `template_name`, `standard_authority`, `description`, `default_auto_approval`, `default_threshold`, `is_system_built_in`, `is_active`) VALUES
('tpl-001', 'JR_T_0197_2020', '金融数据安全 数据安全分级指南', '中国人民银行 / JR/T 0197-2020', '针对证券、基金、银行金融资产数据安全等级规范 (L1~L5)', 1, 0.92, 1, 1),
('tpl-002', 'GB_T_35273_2020', '个人信息安全规范', '国家市场监督管理总局 / GB/T 35273-2020', '个人敏感信息与个人一般信息三层漏斗识别特征库', 1, 0.95, 1, 1);
...
INSERT IGNORE INTO `sec_security_grade` (`id`, `grade_name`, `grade_code`, `sensitivity_score`, `color_tag`, `description`) VALUES
(1, '公开数据 (L1)', 'L1', 10, 'green', '可在企业内部或对外公开披露的数据资产'),
(2, '内部数据 (L2)', 'L2', 30, 'blue', '限企业内部员工及系统授权访问的一般业务数据'),
(3, '敏感数据 (L3)', 'L3', 60, 'orange', '泄露后可能对个人隐私或业务利益造成中等危害的数据'),
(4, '高度敏感数据 (L4)', 'L4', 85, 'volcano', '包含银行卡号、身份证号、鉴权密钥等高危核心数据'),
(5, '极度机密数据 (L5)', 'L5', 100, 'red', '商业核心机密、监管限制、交易底账等顶尖机密');
...
INSERT IGNORE INTO `sec_category_tree` (`id`, `parent_id`, `node_name`, `node_path`, `depth_level`, `visibility`, `admins`, `description`, `sort_order`) VALUES
(1001, 0, '个人信息域', '/个人信息域', 1, 'PUBLIC', '["sec_admin"]', '自然人个人隐私与身份信息', 1),
(1002, 0, '证券交易业务域', '/证券交易业务域', 1, 'PUBLIC', '["sec_admin"]', '证券、基金委托、清算结算核心业务数据', 2),
(1003, 1001, '个人身份鉴权与凭据', '/个人信息域/个人身份鉴权与凭据', 2, 'PUBLIC', '["sec_admin"]', '身份证、护照、生物识别特征及密码哈希', 1),
(1004, 1001, '个人财产与金融账户', '/个人信息域/个人财产与金融账户', 2, 'PUBLIC', '["sec_admin"]', '银行卡号、证券账户号、资金流水', 2);
...
INSERT IGNORE INTO `sec_data_category` (`id`, `category_name`, `category_code`, `tree_node_id`, `security_grade_id`, `priority`, `status`, `description`) VALUES
(10001, '居民身份证号', 'CAT_ID_CARD', 1003, 4, 1, 'ENABLED', '中国大陆18位居民身份证号码'),
(10002, '银行卡号/信用卡号', 'CAT_BANK_CARD', 1004, 4, 1, 'ENABLED', '个人银行借记卡/贷记卡账户卡号'),
(10003, '手机号码', 'CAT_MOBILE_PHONE', 1003, 3, 2, 'ENABLED', '11位大陆移动电话联系方式'),
(10004, '证券资金账户', 'CAT_SECURITY_ACC', 1004, 4, 1, 'ENABLED', '证券集中交易资金账号');
...
-- =====================================================================
-- 8. AI Agent 上下文与 MCP 工具网关 (AI Context Layer)
-- =====================================================================
...
-- 结束说明
SELECT 'Metadata Platform database initialization completed successfully.' AS status;
